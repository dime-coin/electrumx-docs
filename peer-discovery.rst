...
In ElectrumX-Dime this is a per-coin property in `lib/coins.py
...
In ElectrumX-Dime, "recently" is taken to be the last 24 hours.  Only one
...
ElectrumX-Dime will connect to the SSL port if both ports are available.
...
ElectrumX-Dime attempts to connect to onion peers through a Tor proxy that
