...
.. note:: A client or server should only indicate JSON RPC 2.0 by
  ...
  its entirety.  ElectrumX-Dime does and will expect clients advertizing so
...
For example, the legacy Dimecoin address from the genesis block::
...
.. note:: with 500,000 headers of 80 bytes each, a naïve server
  ...
  provide a single merkle proof.  ElectrumX-Dime implements an optimization
