...
.. envvar:: COIN
  ...
  `lib/coins.py`_. For example, ``Dimecoin``.
...
.. envvar:: DAEMON_URL
  ...
  ElectrumX-Dime will initially connect to the first, and failover to
...
.. note:: ElectrumX-Dime will not serve any incoming connections until it has fully caught up
          with your dimecoin daemon.  The only exception is local **RPC** connections,
          which are served at any time after the server has initialized.

.. envvar:: SERVICES

  A comma-separated list of services ElectrumX-Dime will accept incoming connections for.
  ...
  ElectrumX-Dime will not listen for any incoming connections.
...
.. envvar:: REPORT_SERVICES
  ...
  A comma-separated list of services ElectrumX-Dime will advertize and other servers in the
...
.. envvar:: BANNER_FILE
  ...
  + ``$SERVER_VERSION`` is replaced with the ElectrumX-Dime version you are
    running, such as ``1.0.10``.
  + ``$SERVER_SUBVERSION`` is replaced with the ElectrumX-Dime user agent
    string.  For example, ``ElectrumX-Dime 1.0.10``.
  ...
  + ``$DAEMON_SUBVERSION`` is replaced with the daemon's user agent
    string.  For example, ``/DimecoinUnlimited:0.12.1(EB16; AD4)/``.
...
.. envvar:: REORG_LIMIT
  ...
  reorganisation.  ElectrumX-Dime retains some fairly compact undo
  information for this many blocks in levelDB.  The default is a
  function of :envvar:`COIN` and :envvar:`NET`; for Dimecoin mainnet it
  is 200.
...
.. envvar:: MAX_RECV
  ...
  Note that the smallest sane/safe value for Dimecoin is ~800,100 bytes,
...
.. envvar:: MAX_SEND
  ...
  Dimecoin mainnet are over 500KB in size.
...
.. envvar:: SESSION_GROUP_BY_SUBNET_IPV6
  ...
  to process it.  Factors include whether the request uses dimecoind, how much bandwidth
  ...
  If a session disconnects, ElectrumX-Dime continues to associate its cost with its IP address,
...
In response to the :func:`server.peers.subscribe` RPC call, ElectrumX-Dime
will only return peer servers that it has recently connected to and
verified basic functionality.

If you are not running a Tor proxy ElectrumX-Dime will be unable to connect
...
to be running a Tor proxy for ElectrumX-Dime to use.

ElectrumX-Dime will perform peer-discovery by default and announce itself
...
.. envvar:: PEER_DISCOVERY
  ...
  If ``on``, ElectrumX-Dime will occasionally connect to and verify its
...
.. envvar:: PEER_ANNOUNCE
  ...
  If not defined, or non-empty, ElectrumX-Dime will announce itself to
  peers.

  If peer discovery is disabled this environment variable has no
  effect, because ElectrumX-Dime only announces itself to peers when doing
...
.. envvar:: TOR_PROXY_PORT
  ...
  The port on which the Tor proxy is running.  If not set, ElectrumX-Dime
